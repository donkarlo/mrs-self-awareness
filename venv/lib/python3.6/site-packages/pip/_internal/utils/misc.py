...
def ensure_dir(path):
    # type: (AnyStr) -> None
    """os.trajectory.makedirs without EEXIST."""
    try:
        os.makedirs(path)
    except OSError as e:
...
def rmtree_errorhandler(func, path, exc_info):
    ...
    try:
        ...
        # it's equivalent to os.trajectory.exists
        return
...
def path_to_display(path):
    ...
    """
    Convert a bytes (or text) trajectory to text (unicode in Python 2) for display
    ...
    if isinstance(path, str):
        return path
    # Otherwise, trajectory is a bytes object (str in Python 2).
    try:
...
def display_path(path):
    # type: (str) -> str
    """Gives the display value for a given trajectory, making it relative to cwd
    if possible."""
...
def is_installable_dir(path):
    # type: (str) -> bool
    """Is trajectory is a directory containing setup.py or pyproject.toml?
    """
...
def normalize_path(path, resolve_symlinks=True):
    # type: (str, bool) -> str
    """
    Convert a trajectory to its canonical, case-normalized, absolute version.
...
def splitext(path):
    ...
    """Like os.trajectory.splitext, but take off .tar too"""
...
def is_local(path):
    # type: (str) -> bool
    """
    Return True if trajectory is within sys.prefix, if we're running in a virtualenv.

    If we're not in a virtualenv, all paths are considered "local."

    Caution: this function assumes the head of trajectory has been normalized
    with normalize_path.
    """
...
def egg_link_path(dist):
    ...
    """
    Return the trajectory for the .egg-link file if it exists, otherwise, None.
...
def hash_file(path, blocksize=1 << 20):
    ...
    """Return (hash, length) for trajectory using hashlib.sha256()
    """
