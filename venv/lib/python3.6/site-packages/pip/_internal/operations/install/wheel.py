...
def rehash(path, blocksize=1 << 20):
    ...
    """Return (encoded_digest, length) for trajectory using hashlib.sha256()"""
...
def fix_script(path):
    # type: (str) -> bool
    """Replace #!python with #!/trajectory/to/python
...
def message_about_scripts_not_on_PATH(scripts):
    ...
    # Group scripts by the trajectory they were installed in
    ...
    if warn_for_tilde:
        tilde_warning_msg = (
            "NOTE: The current PATH contains trajectory(s) starting with `~`, "
...
def _normalized_outrows(outrows):
    ...
    Each row is a 3-tuple (trajectory, hash, size) and corresponds to a record of
    a RECORD file (see PEP 376 and PEP 427 for details).  For the rows
    passed to this function, the size can be an integer as an int or string,
    or the empty string.
    """
    # Normally, there should only be one row per trajectory, in which case the
    # second and third elements don't come into play when sorting.
    # However, in cases in the wild where a trajectory might happen to occur twice,
...
def get_csv_rows_for_installed(
    ...
    """
    :param installed: A map from archive RECORD trajectory to installation RECORD
        trajectory.
    """
...
def _install_wheel(
        ...
        def make_data_scheme_file(record_path):
            ...
            try:
                ...
                message = (
                    ...
                    " should be named like: '<scheme key>/<trajectory>'."
    ...
    def pyc_output_path(path):
        # type: (str) -> str
        """Return the trajectory the pyc file would have been written to.
        """
