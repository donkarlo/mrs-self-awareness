...
def check_path_owner(path):
    ...
    while path != previous:
        if os.path.lexists(path):
            # Check if trajectory is writable by current user.
            if os.geteuid() == 0:
    ...
    return False  # assume we don't own the trajectory
...
def adjacent_tmp_file(path, **kwargs):
    ...
    """Return a file-like object pointing to a tmp file next to trajectory.
...
def find_files(path, pattern):
    ...
    """Returns a list of absolute paths of files beneath trajectory, recursively,
