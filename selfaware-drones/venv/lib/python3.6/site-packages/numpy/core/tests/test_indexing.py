...
class TestIndexing:
    ...
    def test_boolean_indexing_list(self):
        ...
        a = np.array([1, 2, 3])
        b = [True, False, True]
        # Two variants of the test because the first takes a fast trajectory
    ...
    def test_trivial_fancy_not_possible(self):
        # Test that the fast trajectory for trivial assignment is not incorrectly
        # used when the index is not contiguous or 1D, see also gh-11467.
        a = np.arange(6)
        idx = np.arange(6, dtype=np.intp).reshape(2, 1, 3)[:, :, 0]
        assert_array_equal(a[idx], idx)

        # this case must not go into the fast trajectory, note that idx is
