...
# Variance of the obs
# This is used only as an obs covariance at the first time step.
...
for i in range(dataLength):
    ...
    for n in range(N):
        ...
        if i == firstIt:
            ...
            # Covariance of obs, from the vars of the image
            ...
            # We also update the covariance filter:
        ...
        lambda_after_state = calculateLamdaS(nSuperStates, currState, averageNodes, covGeneralizedObservation,
                                             covSuperstates, skewValue)
        ...
        if oldDistanceMethod == False:
            ...
            probDistDeriv, distanceDeriv = nodesProb(currStateDeriv, averageDiv,
                                                     radiusDeriv * np.ones(len(radiusDeriv)))
        ...
        if i > firstIt:
            ...
            # 1 : state (mus)
            onlyStateAnomaly[n, i] = np.mean(
                np.abs(normalizedPredictedState[0:numState] - normalizedUpdatedState[0:numState]))
            if winningMetric == 1:
                weightsPar[n] = weightscoeff_n[n, i - 1] / (onlyStateAnomaly[n, i] + 1e-10)

            # 2 : error/velocity (err)
            onlyErrorAnomaly[n, i] = np.mean(
                np.abs(normalizedPredictedState[numState:numState * 2] - normalizedUpdatedState[numState:numState * 2]))
            if winningMetric == 2:
            ...
            if n == N - 1:
                ...
                lambda_ = calculateLamdaS(nSuperStates, currMeas, averageNodes, covGeneralizedObservation,
                                          covSuperstates, skewValue)
                ...
                PPwinner = transitionMat[int(activeNodes[int(index_innov_min), i - 1]),
                           :] + 1e-20  # add 1e-100 since KLD doesnt allow zero values
                QQ = lambda_;
    ...
    nameFile = 'results_test_' + '_odometry_' + '_metric' + str(winningMetric) + '_particles' + str(
        nParticles) + '_skew' + str(skewValue) + '_thresh' + str(active_node_threshold) + '_testing' + str(testing)
